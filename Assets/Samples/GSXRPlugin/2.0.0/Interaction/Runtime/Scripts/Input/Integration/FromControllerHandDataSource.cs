...
{
    public class FromControllerHandDataSource : DataSource<HandDataAsset>
    {
        [SerializeField]
        ...
        {
            _skeleton = XRHandSkeleton.CreateSkeletonData(_handedness);
        ...
        protected override void UpdateData()
        {
            ...
            _handDataAsset.IsDominantHand =
                XRInput.GetDominantHand() ==  Handedness.Left
                && _handedness == Handedness.Left
                || (XRInput.GetDominantHand() == Handedness.Right
                    && _handedness == Handedness.Right);

            float indexStrength = _pinchCurve.Evaluate(XRInput.Get(XRInput.Axis1D.PrimaryIndexTrigger, _Controller));
            float gripStrength = _pinchCurve.Evaluate(XRInput.Get(XRInput.Axis1D.PrimaryHandTrigger, _Controller));
            ...
            _handDataAsset.PointerPose = new Pose(
                XRInput.GetLocalControllerPosition(_Controller),
                XRInput.GetLocalControllerRotation(_Controller));
            ...
            Pose pose = new Pose(_ControllerAnchor.position, _ControllerAnchor.rotation);
        ...
        public void InjectAllFromControllerHandDataSource(UpdateModeFlags updateMode, IDataSource updateAfter,
